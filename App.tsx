...
export default function App() {
  ...
  const handleGameOver = (finalScore: number, win: boolean) => {
    ...
    setPlayerState(prev => {
        ...
        if (win) {
            newState.unlockedLevels = Math.max(newState.unlockedLevels, currentLevelId + 1);
  ...
  const renderScreen = () => {
    switch (screen) {
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="flex items-center justify-between mb-8 border-b border-slate-700 pb-4">
                        <div className="flex items-center gap-4">
                            <Trophy size={40} style={{ color: currentWallpaper.primaryColor }} />
                            <div>
                                <h2 className="text-4xl font-display neon-text-shadow" style={{ color: currentWallpaper.primaryColor }}>RANKING GLOBAL</h2>
                                <p className="text-slate-400 text-sm mt-1">Lista dos melhores operadores reais cadastrados no sistema.</p>
                            </div>
                        </div>
                            ...
                            <tbody>
                                {loadingLeaderboard ? (
                                    ...
                                    leaderboardData.map((entry, index) => (
                                        <tr 
                                            key={index} 
                                            className={`border-b border-slate-700/50 last:border-0 ${entry.isUser ? 'bg-white/10' : 'hover:bg-white/5'}`}
                                        >
                                            ...
                                            <td className={`p-4 font-bold ${entry.isUser ? 'text-yellow-400' : 'text-white'}`}>
                                                {entry.name} {entry.isUser && <span className="text-xs ml-2 bg-yellow-500 text-black px-1 rounded">VOCÊ</span>}
                                            </td>
                                            <td className="p-4 text-center">
                                                <div className="flex items-center justify-center gap-2 bg-black/30 py-1 px-3 rounded-full border border-slate-700/50">
                                                    <span className="text-lg">{entry.rank?.icon}</span>
                                                    <span className="font-display text-xs font-bold" style={{ color: entry.rank?.color }}>
                                                        {entry.rank?.name}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="p-4 text-right font-display text-xl" style={{ color: currentWallpaper.primaryColor }}>
          ...
          return (
            <div className="absolute inset-0 bg-slate-900/95 z-20 overflow-y-auto p-8 backdrop-blur-md">
                <div className="max-w-4xl mx-auto">
                    <div className="flex items-center justify-between mb-8 border-b border-slate-700 pb-4">
                        <div className="flex items-center gap-4">
                            <Beaker size={40} style={{ color: currentWallpaper.primaryColor }} />
                            <div>
                                <h2 className="text-4xl font-display neon-text-shadow" style={{ color: currentWallpaper.primaryColor }}>LABORATÓRIO</h2>
                                <p className="text-slate-400 text-sm mt-1">Invista créditos em melhorias definitivas. O custo triplica a cada nível.</p>
                            </div>
                        </div>
                                ...
                                <div key={upgrade.id} className="bg-slate-800/50 border border-slate-700 p-6 rounded-xl flex flex-col items-center text-center hover:border-white transition-colors relative overflow-hidden group">
                                    ...
                                    <div className="flex gap-1 mb-6 relative z-10">
                                        {Array.from({ length: upgrade.maxLevel }).map((_, i) => (
                                            <div key={i} className={`w-3 h-3 rounded-full ${i < currentLevel ? 'shadow-glow' : 'bg-slate-700'}`} 
                                                 style={{ backgroundColor: i < currentLevel ? currentWallpaper.primaryColor : undefined }}
                                            />
                                        ))}
                                    </div>

                                    {isMaxed ? (
                                        <div className="bg-slate-700/50 w-full py-3 rounded text-slate-300 font-mono relative z-10">NÍVEL MÁXIMO</div>
                                    ) : (
                                        <button 
                                            onClick={() => handleUpgradeBuy(upgrade.id, nextPrice)}
                                            disabled={!canAfford}
                                            className={`relative z-10 w-full py-3 rounded font-bold font-mono transition-all flex items-center justify-center gap-2
                                                ${canAfford ? 'bg-yellow-500 hover:bg-yellow-400 text-black' : 'bg-slate-700 text-slate-500 cursor-not-allowed'}
                                            `}
                                        >
                  ...
                  return (
                    <button
                      ...
                      className={`aspect-square border-2 transition-all flex flex-col items-center justify-center group relative overflow-hidden rounded-lg
                         ...
                         ${isNext ? 'shadow-[0_0_10px_rgba(255,255,255,0.3)] animate-pulse' : ''}
                      `}
                      style={{ 
                          borderColor: isNext ? currentWallpaper.primaryColor : (unlocked ? undefined : undefined),
                      }}
                    >
                      {unlocked ? (
                          <>
                            <span className={`text-2xl font-display mb-1 relative z-10 ${isNext ? '' : 'text-slate-400 group-hover:text-white'}`}
                                  style={{ color: isNext ? currentWallpaper.primaryColor : undefined }}>
                                {level.id}
                            </span>
                             <div className="text-[9px] text-slate-500 uppercase relative z-10">{level.pathType}</div>
                            {highScore > 0 && <div className="text-[10px] text-yellow-400 mt-1 font-mono">★ {highScore}</div>}
                          </>
                      ) : (
      ...
      case GameScreen.PLAYING:
        return (
          <>
            <div className="absolute inset-0 pointer-events-none z-30 opacity-50" style={{
                 ...
                 <div className="absolute inset-0 z-[70] bg-black/80 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in duration-300">
                    <div className="bg-slate-900 border-2 border-slate-500 p-8 rounded-xl shadow-2xl max-w-md w-full relative overflow-hidden flex flex-col items-center text-center">
                        ...
                        <h2 className="text-xl font-display tracking-widest text-cyan-400 mb-6 border-b border-slate-700 pb-2 w-full">
                            INICIALIZAÇÃO DO SISTEMA ({tutorialStep + 1}/{TUTORIAL_STEPS.length})
                        </h2>
                        
                        <div className="flex-1 flex flex-col items-center justify-center min-h-[200px] mb-6">
                            {TUTORIAL_STEPS[tutorialStep].icon}
                            <h3 className="text-2xl font-bold text-white mb-4">{TUTORIAL_STEPS[tutorialStep].title}</h3>
                            <p className="text-slate-300">{TUTORIAL_STEPS[tutorialStep].description}</p>
                        </div>
                        
                        <Button onClick={handleNextTutorialStep}>
                            <div className="flex items-center gap-2">
                                {tutorialStep < TUTORIAL_STEPS.length - 1 ? 'PRÓXIMO' : 'INICIAR MISSÃO'} <ArrowRight size={20} />
                            </div>
                        </Button>
                    </div>
                 </div>
            ...
            {isPaused && !showTutorial && (
                <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in duration-200">
                    <div className="bg-slate-900 border-2 border-slate-700 p-8 rounded-xl shadow-2xl max-w-sm w-full relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1" style={{ background: currentWallpaper.primaryColor }}></div>
                        
                        <h2 className="text-3xl font-display text-center mb-8 neon-text-shadow tracking-wider" style={{ color: currentWallpaper.primaryColor }}>
                            SISTEMA SUSPENSO
                        </h2>
                        
                        <div className="flex flex-col gap-4">
                            <Button onClick={togglePause}>
                                <div className="flex items-center gap-2"><Play size={20} /> CONTINUAR</div>
                            </Button>
                            ...
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    className={`flex-1 py-3 border rounded font-display text-sm flex items-center justify-center gap-2 transition-all
                                        ${playerState.settings.musicVolume ? 'bg-slate-800 border-slate-600 text-white hover:bg-slate-700' : 'bg-black border-red-900 text-red-700'}
                                    `}
                                >
                                    {playerState.settings.musicVolume ? <Music size={18} /> : <div className="relative"><Music size={18} /><div className="absolute inset-0 border-t border-red-500 rotate-45 top-2"></div></div>}
                                    MÚSICA
                                </button>
                                <button 
                                    onClick={toggleSFX}
                                    className={`flex-1 py-3 border rounded font-display text-sm flex items-center justify-center gap-2 transition-all
                                        ${playerState.settings.sfxVolume ? 'bg-slate-800 border-slate-600 text-white hover:bg-slate-700' : 'bg-black border-red-900 text-red-700'}
                                    `}
                                >
                                    {playerState.settings.sfxVolume ? <Volume2 size={18} /> : <VolumeX size={18} />}
                                    SONS
                                </button>
                            </div>

                            <Button variant="danger" onClick={quitToMenu}>
                                <div className="flex items-center gap-2"><LogOut size={20} /> ABORTAR MISSÃO</div>
                            </Button>
                        </div>
                    </div>
                </div>
                     ...
                     return (
                        <div key={item.id} className="group relative flex items-center justify-end">
                            ...
                            <button 
                                ...
                                `}
                                style={{ borderColor: owned > 0 ? currentWallpaper.primaryColor : undefined }}
                            >
                                <div className="text-xl md:text-2xl mb-1">{item.icon}</div>
                                {owned > 0 ? (
                                    <div className="absolute -top-2 -right-2 text-black font-bold w-5 h-5 rounded-full flex items-center justify-center text-xs shadow-glow"
                                         style={{ backgroundColor: currentWallpaper.primaryColor }}>
                                        {owned}
                                    </div>
                                ) : (
