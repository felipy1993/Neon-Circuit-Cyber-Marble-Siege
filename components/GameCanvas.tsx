...
const GameCanvas = forwardRef<GameCanvasRef, GameCanvasProps>(({ 
  ...
  const animate = useCallback((time: number) => {
    ...
    projectilesRef.current.forEach(p => {
        if (p.isEmp) {
             ...
             const tempMarble: Marble = { id: '', color: p.color, type: MarbleType.NORMAL, offset: 0, speed: 0, backwards: false };
  ...
  const drawCyberpunkBackground = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      backgroundNodesRef.current.forEach((node, i) => {
          ctx.fillStyle = i % 2 === 0 ? wallpaper.primaryColor : wallpaper.secondaryColor;
  ...
  const drawShooter = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    if (empNextShotRef.current) {
         ...
         const dummy: Marble = { id: '', color: currentShooterColorRef.current, type: MarbleType.NORMAL, offset: 0, speed: 0, backwards: false };
    ...
    const nextDummy: Marble = { id: '', color: nextMarbleColorRef.current, type: MarbleType.NORMAL, offset: 0, speed: 0, backwards: false };
